...
from app import ERROR, __application__, __version__, config, database, todo
...
def init(
    ...
    if app_init_error:
        typer.secho(
            f'Creating config file failed with "{ERROR[app_init_error]}"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    ...
    if db_init_error:
        typer.secho(
            f'Creating database failed with "{ERROR[db_init_error]}"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    else:
...
def add(
    ...
    if error:
        typer.secho(
            f'Adding to-do failed with "{ERROR[error]}"', fg=typer.colors.RED
        )
        raise typer.Exit(1)
    else:
        typer.secho(
...
def set_done(todo_id: int = typer.Argument(...)) -> None:
    ...
    if error:
        typer.secho(
            f'Completing to-do # "{todo_id}" failed with "{ERROR[error]}"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    else:
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"{__application__} v{__version__}")
        raise typer.Exit()
